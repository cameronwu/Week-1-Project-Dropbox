...
//  Dropbox
//
//  Created by Cameron Wu on 9/20/15.
...
class ExistingFavoritesViewController: UIViewController {
    ...
    override func viewDidAppear(animated: Bool) {
        if (defaults.boolForKey("isGettingStartedFavorited")) {
            // If isGettingStartedFavorited is true, display the files
            favoritesImageView.image = UIImage(named: "ExistingFavorites")
            gettingStartedButton.hidden = false
        } else {
            // If isGettingStartedFavorited is false, hide the files
            favoritesImageView.image = UIImage(named: "EmptyFavorites")
            gettingStartedButton.hidden = true
        }
    }
